...
function isCreditCardNumber(ccn) {
  ...
  ccn = ccn.toString();
  if (!ccn.length) return false;
  // eslint-disable-next-line no-param-reassign
  if (ccn.length % 2 !== 0) ccn = `0${ccn}`;
  let sum = 0;
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 1; i < pathes.length; i++) {
    ...
    const commonPathParts = commonPath.split('/');

    // Find the common path
    commonPath = '';
    for (let j = 0; j < currentPathParts.length; j++) {
      if (currentPathParts[j] === commonPathParts[j]) {
        commonPath += `${currentPathParts[j]}/`;
      } else {
        break;
      }
    }
